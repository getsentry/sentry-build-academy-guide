...
1. **Install Sentry SDK**
   ...
   ```bash
   pnpm add @sentry/react @sentry/vite-plugin --filter frontend
   ```
...
3. **Initialize Sentry**
   ...
   Create `./apps/frontend/src/instrument.js`:
...
5. **Create your ErrorBoundary**
   ...
   Create `./apps/server/src/components/ErrorBoundary.ts`:
...
4. **Update the esbuild configuration to include Sentry**
...
5. **Move through the Source Map configuration**

   ```bash
   cd apps/server && npx @sentry/wizard@latest -i sourcemaps
   ```
